...
function chose(x) {
    ...
    $(x).on("click", function () {
        ...
        let word = document.querySelector(x).childNodes[0].innerHTML
        let event  =questions[word]
        let randomNumber = Math.floor(Math.random() * event.length )
        let random = event[randomNumber]
        let question = random.question;
        let answer = random.answer;
        console.log( questions[word]);
        questions[word].splice(randomNumber, 1);
        console.log( questions[word]);
        if (changePlayer == 0) {
...
var alphabet = ["أ", "ب", "ت", "ث", "ج", "ح", "خ", "د", "ذ", "ر", "ز", "س", "ش", "ص", "ض", "ط", "ظ", "ع", "غ", "ف", "ق", "ك", "ل", "م", "ن", "ه", "و", "ي"].sort(() => Math.random() - 0.5);

Array.from($(".checke")).forEach((ele,i) =>{
    ele.innerHTML = `<h3 style="text-align: center;">${alphabet[i]}</h3>`
})
